...
public class GlideImageLoader extends ImageLoader {
    ...
    public void displayImage(Context context, Object path, ImageView imageView) {
        ...
        final SlideShowActivity.Bean bean = (SlideShowActivity.Bean) path;
        //Glide 加载图片简单用法
        Glide.with(context).load(bean.getImageUrl()).into(imageView);
        imageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(mContext, bean.getContentUrl(), Toast.LENGTH_SHORT).show();
            }
        });
...
//    @Override
//    public ImageView createImageView(Context mContext) {
